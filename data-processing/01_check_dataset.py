...
def check_episode(path: str, min_steps: int, spike_thresh: float,
    ...
    if len(spike_frames):
        issues["spikes"] = (f"{len(spike_frames)} frames exceed "
                            f"{spike_thresh:.3f} rad: {spike_frames[:5].tolist()}")
    return issues
...
def main():
    ...
    results = [(os.path.basename(p),
                check_episode(p, args.min_steps, args.spike_thresh, args.same_thresh))
               for p in files]
